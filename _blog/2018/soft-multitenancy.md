...
high level description on what is required to deploy multiple Istio control planes
...
```sh
dev-admin:~/go/src/istio.io/istio# kubectl get pods -n istio-system
...
dev-admin:~/go/src/istio.io/istio# kubectl get pods -n istio-system1
...
that tenant. As an example, updating the [bookinfo](https://istio.io/docs/guides/bookinfo.html)
...
```sh
dev-admin:~/go/src/istio.io/istio# kubectl get pods -n ns-0
NAME                              READY     STATUS    RESTARTS   AGE
...
```sh
dev-admin:~/go/src/istio.io/istio# kubectl get pods -n ns-1
Error from server (Forbidden): pods is forbidden: User "dev-admin" cannot list pods in the namespace "ns-1"

```

If the [addon tools](https://istio.io/docs/tasks/telemetry/), example
[prometheus](https://istio.io/docs/tasks/telemetry//querying-metrics.html), are deployed
